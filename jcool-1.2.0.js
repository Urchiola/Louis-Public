...
* @Last Modified time: 2016-12-11 17:14:39
*/
...
$$.extend($$,{
        ...
        if(arglen === 1){
            ...
            if($$.isString(ptarg)){
                ...
                if(document.getElementsByClassName){
                    return document.getElementsByClassName(ptarg);
                }else{
                    // 首先大海捞针 ---放在集合里面
            ...
            //如果传递过来的是字符串 ，则转化成元素对象
            if($$.isString(ptarg)){
                ptarg = document.getElementById(ptarg);
            }
            //如果兼容getElementsByClassName,
            //bug:未搜索到则 报错
            if(ptarg.getElementsByClassName){
                return ptarg.getElementsByClassName(clname);
            }else{
                //如果浏览器不支持
                dom = ptarg.getElementsByTagName('*');
                for(var i,len=dom.length;i<len;i++) {
                    if(dom[i].className && dom[i].className ==clname ) {
                        elements.push(dom[i]);
                    }
                }
            }
    ...
    $group:function (arg){
        ...
        for(var i=0,len=arr.length;i<len;i++){
            var firstStr= arr[i].charAt(0)
            if(firstStr==='#'){
                //获取样式的 序列号
                var index= firstStr.indexOf('#') 
                //截取 从序列号+1 的字符串，并返回dom对象
                //bug:之前我们定义的doms是数组，但是$id获取的不是数组，而是单个元素
                //doms= document.getElementById(arr[i].slice(index+1))  //测出获得是 单个标签伪数组对象
                doms=[this.$id(arr[i].slice(index+1))] 
                pushArray(doms,result)
            }else if(firstStr==='.'){
               var index= firstStr.indexOf('.')
                //测出获得是 单个标签伪数组对象
                // 测出获得是 类样式 数组 
                doms=this.$class(arr[i].slice(index+1))
                pushArray(doms,result)
            }else{
                doms=this.$tag(arr[i])
                pushArray(doms,result)
    ...
    $level:function(select){
        ...
        for(var i = 0, len = sel.length; i < len; i++){
            ...
            if(first ==='#'){
                ...
                //console.log("#");
                //console.log([$$.$id(item.slice(index + 1))])
                context = result;
            }else if(first ==='.'){
                //如果是.
                //如果是.
                ...
                if(context.length){
                    for(var j = 0, contextLen = context.length; j < contextLen; j++){
                        pushArray($$.$class( context[j],item.slice(index + 1)));
                    }
                }else{
